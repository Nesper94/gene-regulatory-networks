(*2019.03.07: Voy a cambiar la presentación de los gráficos de manera que la inhibición se represente como una flecha con cabeza plana, según la convención general. Este módulo es básicamente la función DisplayNonWeightedHaploidGraphWithMorphInputs del módulo EvolAlgorithm4PatternFormingGRCModel2MathPackageV9 pero con las modificaciones necesarias para la nueva representación.*)
...
StyleBox[\"B\",\nFontSize->0]\)\)]\)",Black,"\!\(\*SubscriptBox[\(M\), \(\!\(\*
StyleBox[\"C\",\nFontSize->0]\)\)]\)",Black];
EdgeColoringF::usage="Función que hace que una interacción de represión (-1) sea roja y una de activación (1) sea azul."
...
(*2019.03.07: Voy a cambiar la presentación de los gráficos de manera que la inhibición se represente como una flecha con cabeza plana, según la convención general. Bajo esta línea se encuentra la línea original en forma de comentario, la línea que le sigue es la línea modificada*)
...
EdgeRenderingFunction->({
  ...
  Edge2ColorMap[[Flatten[Position[EdgeComponents,#2]][[1]]]],Thickness[#3],Arrow[#1,0.2]}&),
...
(************************************************************************************************************)
(*Esta es la función para graficar las topologías teniendo en cuenta la fuerza de las interacciones*)
...
RegEffectList=Sign[Tags];
{EdgeComponents,Edge2ColorMap}={({#[[1]],#[[2]]}&/@GraphRules),(EdgeColoringF/@RegEffectList)};
(*La siguiente línea fue insertada por mí el 2019-03-15*)
ArrowHeadMap = ArrowheadStyleFunction /@ RegEffectList;
MaxThickness=0.0095;
...
EdgeRenderingFunction->({
ArrowHeadMap[[  Flatten[ Position[EdgeComponents, #2]   ][[1]]   ]],
(*En la siguiente línea se cambia Arrow[#1,0.15] por Arrow[#1,0.2] para que la punta de las flechas no sea tapada por los nodos.*)
  Edge2ColorMap[[Flatten[Position[EdgeComponents,#2]][[1]]]],Thickness[#3],Arrow[#1,0.2]}&),
Method->{"Automatic","Rotation"->2 Pi},VertexRenderingFunction->({EdgeForm[ColorF01[#2]],ColorF01[#2],Disk[#1,0.15],Darker[Gray,0.009],Text[Style[#2,13,Bold,White],#1]}&),ImageSize->300]
]
