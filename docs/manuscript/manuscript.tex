...
fitness criterion used to assess the quality of the phenotype to
...
guide for future synthetic biology implementations.
...
morphogen inputs allows finding a different solution for the patterning
...
GRNs~(\nameref{S6_Fig}), suggesting that an organism could switch
...
interact with any gene, we found a more significant number of possible topologies with the
same striped phenotype and a diverse combination of diffusion and degradation
parameters~(\nameref{S7_Fig}). For example, in a single input model, the I4-FFL could
...
Although it has been experimentally demonstrated that the Incoherent Feed-Forward
Loop type 2 (I2-FFL) can generate a striped gene expression
...
can not always find global minima but local minima or their surroundings. However,
...
algorithm selected similar expression mechanisms that very
different topologies can produce.
...
knowledge the first time a GRN with a topology such as that of topology
nine~(Fig \ref{fig:topol9})
and its expression mechanism is reported, increasing our knowledge of the
genotype-phenotype map in this kind of system. It would be very interesting
...
regulatory networks or identify new ones existing in nature.
